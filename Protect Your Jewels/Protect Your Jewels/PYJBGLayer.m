...
- (id)initWithTheme:(NSString *)theme {
    if (self = [super init]){
        ...
        new_Cloud = [[[PYJCloud alloc] initFrontCloud:(winSize.width/2) withTheme:m_Theme] autorelease];
        [m_FrontCloudArray addObject:new_Cloud];
        [self addChild:new_Cloud z:1];
        ...
        new_Cloud = [[[PYJCloud alloc] initBackCloud:(3*winSize.width/2) withTheme:m_Theme] autorelease];
        [m_BackCloudArray addObject:new_Cloud];
        [self addChild:new_Cloud z:0];
        
        new_Cloud = [[[PYJCloud alloc] initBackCloud:(winSize.width/2) withTheme:m_Theme] autorelease];
        [m_BackCloudArray addObject:new_Cloud];
        [self addChild:new_Cloud z:0];
...
- (void) update:(ccTime)dt
{
    ...
    BOOL spawnFront = NO;
    BOOL spawnBack = NO;
    PYJCloud* toRemove = nil;
    
    for (PYJCloud *m_Cloud in m_FrontCloudArray)
    {
    ...
    for (PYJCloud *m_Cloud in m_BackCloudArray)
    {
        CGPoint prevPosition = m_Cloud.position;
        [m_Cloud update:dt];
...
{
    PYJCloud *new_Cloud = [[[PYJCloud alloc] initFrontCloud:(-m_FrontCloudWidth/2) withTheme:m_Theme] autorelease];
    [m_FrontCloudArray addObject:new_Cloud];
    [self addChild:new_Cloud z:1];
}
...
-(void) spawnBackCloud
{
    PYJCloud *new_Cloud = [[[PYJCloud alloc] initBackCloud:(-m_BackCloudWidth/2) withTheme:m_Theme] autorelease];
    [m_BackCloudArray addObject:new_Cloud];
    [self addChild:new_Cloud z:0];
}
