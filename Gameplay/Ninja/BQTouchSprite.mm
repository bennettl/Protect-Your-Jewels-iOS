//
//  BQTouchSprite.m
//  GamePlay
//
//  Created by Brian Quock on 2/22/14.
//  Copyright (c) 2014 Bennett Lee. All rights reserved.
//

#import "BQTouchSprite.h"

@implementation BQTouchSprite

+ (BQTouchSprite *)touchSprite{
    return [[[self alloc] initWithDynamicBody:@"jewel" spriteFrameName:@"jewel.png"] autorelease];
}

- (id)initWithSpriteLayer:(BLSpriteLayer *)sl{
    if (self = [super initWithDynamicBody:@"jewel"
                          spriteFrameName:@"jewel.png"]){
        
        spriteLayer = sl;  // Store the sprite layer
        
        // Set touch to collide with everything but the jewel
        for (b2Fixture *f = self.body->GetFixtureList(); f; f = f->GetNext()){
            b2Filter tf = f->GetFilterData();
            tf.categoryBits = 0x0008;
            tf.maskBits = 0x0004;
            //tf.groupIndex = -1;
            f->SetFilterData(tf);
        }
        //b2Filter tf = self.body->GetFixtureList()[0].GetFilterData();
        //tf.categoryBits = 0x0008;
        //tf.maskBits = 0xFFFF ^ 0x0004;
        //tf.groupIndex = -1;
        //self.body->GetFixtureList()[0].SetFilterData(tf);
    }
    return self;
}

// Does ccLocation intersect with any of the body's fixtures?
-(BOOL)intersectsWithPoint:(CGPoint)ccLocation{
    b2Vec2 b2Location(ccLocation.x/PTM_RATIO, ccLocation.y/PTM_RATIO);
    
    // Loop through and test all fixtures
    for (b2Fixture *f = self.body->GetFixtureList(); f; f = f->GetNext()){
        if (f->TestPoint(b2Location)){
            return YES;
        }
    }
    return NO;
}

@end
